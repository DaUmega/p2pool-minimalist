...
# ── wait for Tari node gRPC (if merge mining enabled) ────────────────────────
if [ -n "$MERGE_MINE_FLAG" ]; then
    log "Waiting for Tari node gRPC on ${TARI_NODE_HOST}:${TARI_GRPC_PORT} (timeout 120s)..."
    WAIT=0
    until nc -z "${TARI_NODE_HOST}" "${TARI_GRPC_PORT}" 2>/dev/null; do
        sleep 5
        WAIT=$((WAIT+5))
        if [ "$WAIT" -ge 120 ]; then
            log "WARNING: Tari node not reachable after 120s — starting p2pool without merge mining"
            MERGE_MINE_FLAG=""
            break
        fi
    done
    [ -n "$MERGE_MINE_FLAG" ] && log "Tari node gRPC is reachable."
fi
